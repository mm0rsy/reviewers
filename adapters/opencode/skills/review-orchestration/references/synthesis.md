...
4. **Coverage gaps** — changed files no reviewer's patterns matched. List them so review.md
   can be extended.
5. **Project-level pass** — this is where `project`-scope review happens, exactly once.
   Having read every report and the full change set, assess the change against the overall
...
4. `## Project-level observations` — the once-per-review project-scope pass (Step 2, item 5).
5. `## Fix plan by file` — one subsection per file, findings ordered by severity, each entry:
...
6. `## Positive observations` — merged from all reports (deduplicated).
7. `## Appendix` — links to each individual reviewer report.
